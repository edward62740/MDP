...
class RPI_connection:
    ...
    def bluetooth_connect(self):
        logger.debug('Establishing connection with A7 Tablet')
        CHANNEL = 1        
        while True:
            try:
                ...
                logger.debug("MDPGrp22 bluetooth is now discoverable")
                self.android_socket = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
                self.android_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                ...
                logger.debug("Connection with Android failed: " + str(e))
                ...
                CHANNEL+=1
                CHANNEL%=7
                logger.debug(f"Trying channel {CHANNEL}...")
    ...
    def PC_connect(self):    
        logger.debug(f'Establishing connection with PC, creating a server at {self.HOST}:{self.PORT}')
        TRIES = 0
        RETRY = True
        while RETRY and TRIES < RETRY_LIMIT:
            try:
                ...
                logger.debug(f"Connection from {address} established")
                RETRY = False
            except Exception as e:
                logger.debug("Connection with PC failed: " + str(e))
                # Close socket if it exists
                if self.pc_client_sock:
                    self.pc_client_sock.close()
                self.socket.close()
    ...
    def android_receive(self):
        ...
        logger.debug(f"Received message from Android: {self.a_msg}")
        return self.a_msg
    ...
    def PC_receive(self):
        ...
        logger.debug(f"Received message from PC: {self.pc_msg}")
        return self.pc_msg
